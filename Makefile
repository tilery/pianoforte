...
import:
	...
	sed -i.back '/END;/d' tmp/city.sql
	sed -i.back '/COMMIT;/d' tmp/city.sql
	rm tmp/city.sql.back
	...
	# Remove transaction management, as it does not cover the DROP TABLE; we'll cover the transaction manually with "psql --single-transaction"
	sed -i.back '/BEGIN;/d' tmp/country.sql
	sed -i.back '/END;/d' tmp/country.sql
	sed -i.back '/COMMIT;/d' tmp/country.sql
	rm tmp/country.sql.back
...
boundary:
	...
	ogr2ogr --config PG_USE_COPY YES -lco GEOMETRY_NAME=geometry -lco DROP_TABLE=IF_EXISTS -f PGDump tmp/boundary.sql data/boundary.json -select name,'name:en','name:fr','name:ar','name:es','name:de','name:ru','ISO3166-1:alpha2' -nln itl_boundary
	# Remove transaction management, as it does not cover the DROP TABLE; we'll cover the transaction manually with "psql --single-transaction"
